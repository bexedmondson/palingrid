...
func _load_anonymous_stats():
	...
	push_warning("Loading anonymous stats...")
...
func _on_profile_timeout():
	...
	if profile_load_attempts < MAX_PROFILE_LOAD_ATTEMPTS:
		...
		push_warning("Max attempts - using defaults")
		_clear_profile_timeout()
		_stop_profile_polling()
		waiting_for_profile = false
...
func _on_profile_loaded(loaded_nickname: String, score: int, _streak: int, achievements: Array, play_count: int):
	...
	push_warning("Profile loaded: %s (weekly score: %d, plays: %d)" % [loaded_nickname, score, play_count])
	
	# If backend has different nickname (e.g. auto-suffixed), update local storage
	if CheddaBoards.is_anonymous() and not loaded_nickname.is_empty():
		if nickname != loaded_nickname:
			push_warning("Updating local nickname: '%s' -> '%s' (backend sync)" % [nickname, loaded_nickname])
			nickname = loaded_nickname
			_save_player_data()
			#if anonymous_panel and anonymous_panel.visible and anon_welcome_label:
			#	anon_welcome_label.text = "Welcome back, %s!" % loaded_nickname
